  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard title="Entregas Realizadas" value={stats.entregas} icon={Truck} link="/dashboard/entregas" imageId="delivery-made" imageHint="supply truck" loading={loading} />
      </div>
                        ...
                        {recentActivity.map((activity: any) => {
                            ...
                            if (activity.type === 'damnificado') {
                             ...
                             if (activity.type === 'donacion') {
                                const summary = activity.tipo_donacion === 'monetaria' 
                                    ? `una donación monetaria por ${new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }).format(activity.monto || 0)}`
                                    : `una donación de ${activity.articulos?.map((a:any) => a.nombre).join(', ') || 'artículos variados'}.`;
                                return <ActivityItem key={`do-${activity.id}`} icon={Box} text={<>Nueva donación de <span className="font-semibold">{activity.donante}</span>: {summary}</>} time={time} />
                            }
